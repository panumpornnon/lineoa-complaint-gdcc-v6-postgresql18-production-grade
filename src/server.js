...
app.use(cors({
  origin(origin, callback) {
    // 🛠️ แก้ไข: เพิ่มการเช็คให้ข้ามผ่านสำหรับ localhost และโดเมน services.suratcity.go.th
    if (!origin || 
        origin.includes('localhost') || 
        origin.includes('127.0.0.1') || 
        origin.includes('services.suratcity.go.th') || 
...
// new
// ========================================================
// 🔥 เพิ่มจุดที่ 1: ดักตัดพาสย่อยออกระดับ Express ก่อนเข้า Route
// ========================================================
app.use((req, res, next) => {
  // หาก Request วิ่งเข้ามาโดยมีพาส /complaint นำหน้า ให้ตัดออกชั่วคราวเพื่อให้ Express ทำงานถูกโฟลเดอร์
  if (req.url.startsWith('/complaint')) {
    req.url = req.url.substring(10); // ตัดคำว่า '/complaint' (10 ตัวอักษร) ออก
    if (req.url === '') req.url = '/';
  }
  next();
});
...
// new
// ========================================================
// 🔥 เพิ่มจุดที่ 2: ดักส่งไฟล์หน้าแรกในกรณีที่เรียกพาสตัวเปล่า
// ========================================================
app.get('/', (req, res) => {
  // ส่งไฟล์หน้าแรกสุดของคุณออกไปโดยตรง (ตัวอย่างนี้อิงตามชื่อไฟล์ complaint.html)
  res.sendFile(path.join(publicPath, 'index.html')); 
  // 💡 หมายเหตุ: หากไฟล์หน้าแรกในโฟลเดอร์ public ของคุณชื่อ index.html ให้แก้คำว่า 'complaint.html' เป็น 'index.html'
});
// ========================================================
